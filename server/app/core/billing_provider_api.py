...
async def verified_provider_webhook(provider: str, request: Request, x_billing_signature: str | None = Header(default=None, alias="X-Billing-Signature")) -> dict[str, Any]:
    ...
    return _apply_verified(provider, verified, billing_service, store)
...
async def stripe_sandbox_webhook(
    ...
    if not stripe_signature:
        raise HTTPException(status_code=401, detail="STRIPE_WEBHOOK_SIGNATURE_REQUIRED")
    body = await request.body()
    if len(body) > 262_144:
        raise HTTPException(status_code=413, detail="STRIPE_WEBHOOK_BODY_TOO_LARGE")
    try:
        adapter = configured_stripe_sandbox_adapter()
        verified = adapter.verify_webhook(body, stripe_signature)
        ...
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return _apply_verified("stripe", verified, billing_service, store)


def _apply_verified(provider: str, verified: Any, billing_service: Any, store: Any) -> dict[str, Any]:
    try:
...
def billing_features(request: Request, authorization_header: str = Header(..., alias="Authorization"), x_request_id: str | None = Header(default=None, alias="X-Request-ID")) -> dict[str, Any]:
    from app.main import authorize_request, billing_service, principal_from_token, request_id, require_bearer
