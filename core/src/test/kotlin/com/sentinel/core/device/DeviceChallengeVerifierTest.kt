...
class DeviceChallengeVerifierTest {
    ...
    private val testNow = Instant.parse("2026-08-10T12:00:00Z")

    private fun challenge(
        id: String = "challenge-1",
        nonce: ByteArray,
        expectedFingerprint: String? = null,
        expiresAt: Instant = testNow.plusSeconds(60)
    ) = IssuedDeviceChallenge(id, nonce, expiresAt, expectedFingerprint)
    ...
    @Test
    fun expiredChallengeIsRejectedBeforeSignatureVerification() {
        val keyPair = generateP256KeyPair()
    ...
    fun unknownChallengeIsRejected() {
        val keyPair = generateP256KeyPair()
        val verifier = DeviceChallengeVerifier(TestChallengeStore(null), RecordingAuditSink()) { testNow }
        val result = verifier.verify(PresentedDeviceProof("missing", keyPair.public.encoded, byteArrayOf(1, 2, 3)))
        assertEquals(DeviceVerificationResult.Rejected(DeviceVerificationFailure.CHALLENGE_NOT_FOUND), result)
    }
    ...
    fun replayIsRejectedAfterValidSignature() {
        ...
        val store = TestChallengeStore(challenge(nonce = nonce), consumed = true)
        val verifier = DeviceChallengeVerifier(store, RecordingAuditSink()) { testNow }
        val result = verifier.verify(PresentedDeviceProof("challenge-1", keyPair.public.encoded, sign(keyPair, nonce)))
        assertEquals(DeviceVerificationResult.Rejected(DeviceVerificationFailure.CHALLENGE_REPLAYED), result)
    }

    @Test
    fun shortChallengeIsRejected() {
        val keyPair = generateP256KeyPair()
        val store = TestChallengeStore(challenge(nonce = ByteArray(16)))
        val verifier = DeviceChallengeVerifier(store, RecordingAuditSink()) { testNow }
        val result = verifier.verify(PresentedDeviceProof("challenge-1", keyPair.public.encoded, byteArrayOf(1)))
        assertEquals(DeviceVerificationResult.Rejected(DeviceVerificationFailure.MALFORMED_REQUEST), result)
    }

    @Test
    fun oversizedProofIsRejectedBeforeCryptoWork() {
        val store = TestChallengeStore(null)
        val verifier = DeviceChallengeVerifier(store, RecordingAuditSink()) { testNow }
        val result = verifier.verify(PresentedDeviceProof("challenge-1", ByteArray(513), ByteArray(1)))
        assertEquals(DeviceVerificationResult.Rejected(DeviceVerificationFailure.MALFORMED_REQUEST), result)
    }
