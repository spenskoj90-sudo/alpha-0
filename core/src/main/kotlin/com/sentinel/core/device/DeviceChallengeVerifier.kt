...
enum class DeviceVerificationFailure {
    MALFORMED_REQUEST, CHALLENGE_NOT_FOUND, CHALLENGE_EXPIRED, UNSUPPORTED_KEY,
    INVALID_SIGNATURE, DEVICE_MISMATCH, CHALLENGE_REPLAYED, CHALLENGE_STORE_UNAVAILABLE, AUDIT_UNAVAILABLE
}
...
class DeviceChallengeVerifier(
    ...
    fun verify(proof: PresentedDeviceProof): DeviceVerificationResult {
        if (proof.challengeId.isBlank() || proof.challengeId.length > MAX_CHALLENGE_ID_LENGTH ||
        ...
        val challenge = try { challengeStore.find(proof.challengeId) } catch (_: Exception) {
        ...
        if (challenge.id != proof.challengeId || challenge.id.isBlank() || challenge.id.length > MAX_CHALLENGE_ID_LENGTH ||
        ...
        val publicKey = try {
            val key = KeyFactory.getInstance(EC_ALGORITHM).generatePublic(X509EncodedKeySpec(proof.publicKeyEncoded))
            key as? ECPublicKey ?: return reject("unknown", DeviceVerificationFailure.UNSUPPORTED_KEY)
        ...
        if (challenge.expectedFingerprint != null && !MessageDigest.isEqual(
                challenge.expectedFingerprint.toByteArray(Charsets.US_ASCII), fingerprint.toByteArray(Charsets.US_ASCII))) {
            return reject(fingerprint, DeviceVerificationFailure.DEVICE_MISMATCH)
        }
        val validSignature = try {
            Signature.getInstance(SIGNATURE_ALGORITHM).apply { initVerify(publicKey); update(challenge.nonce) }.verify(proof.signature)
        ...
        val consumed = try { challengeStore.consume(proof.challengeId) } catch (_: Exception) {
    ...
    private fun fingerprint(encodedPublicKey: ByteArray): String =
        MessageDigest.getInstance(HASH_ALGORITHM).digest(encodedPublicKey).joinToString("") { "%02x".format(it.toInt() and 0xff) }
