from __future__ import annotations

import hashlib
import os
from pathlib import Path

import psycopg

DATABASE_URL = os.environ["DATABASE_URL"]
MIGRATIONS = Path(__file__).resolve().parent / "migrations"


def psycopg_url(url: str) -> str:
    """Convert SQLAlchemy's driver-qualified URL to a psycopg conninfo URL."""
    return url.replace("postgresql+psycopg://", "postgresql://", 1)


def _enable_service_role(cur: psycopg.Cursor) -> None:
    # Transaction-local by design: compatible with PgBouncer transaction
    # pooling and automatically cleared on commit/rollback.
    cur.execute("SELECT set_config('app.service_role', 'true', true)")


def main() -> None:
    with psycopg.connect(psycopg_url(DATABASE_URL)) as conn:
        with conn.transaction():
            with conn.cursor() as cur:
                _enable_service_role(cur)
                cur.execute("CREATE TABLE IF NOT EXISTS schema_migrations (version text PRIMARY KEY, checksum text NOT NULL, applied_at timestamptz NOT NULL DEFAULT now())")
                cur.execute("SELECT version, checksum FROM schema_migrations")
                applied = dict(cur.fetchall())

        for path in sorted(MIGRATIONS.glob("*.sql")):
            version = path.stem
            checksum = hashlib.sha256(path.read_bytes()).hexdigest()
            if version in applied:
                if applied[version] != checksum:
                    raise RuntimeError(f"Migration checksum mismatch: {version}")
                continue
            with conn.transaction():
                with conn.cursor() as cur:
                    _enable_service_role(cur)
                    cur.execute(path.read_text(encoding="utf-8"))
                    cur.execute("INSERT INTO schema_migrations(version, checksum) VALUES (%s, %s)", (version, checksum))
            applied[version] = checksum


if __name__ == "__main__":
    main()
