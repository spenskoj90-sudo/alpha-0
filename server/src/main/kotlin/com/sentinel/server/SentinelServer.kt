...
fun main() {
    ...
    server.createContext("/healthz") { exchange ->
        if (exchange.requestMethod != "GET") respond(exchange, 405, "{\"error\":\"method_not_allowed\"}") else respond(exchange, 200, "{\"status\":\"ok\"}")
    }
    server.createContext("/v1/devices/register") { exchange ->
        if (exchange.requestMethod != "POST") respond(exchange, 405, "{\"error\":\"method_not_allowed\"}") else runCatching {
        ...
        if (exchange.requestMethod != "POST") respond(exchange, 405, "{\"error\":\"method_not_allowed\"}") else runCatching {
            ...
            when (val result = verifier.verify(proof)) {
                is DeviceVerificationResult.Verified -> {
                    val credential = sessions.issue(result.fingerprint, Duration.ofMinutes(30)) ?: return@runCatching respond(exchange, 503, "{\"error\":\"session_unavailable\"}")
    ...
    server.createContext("/v1/protected/ping") { exchange ->
        if (exchange.requestMethod != "GET") respond(exchange, 405, "{\"error\":\"method_not_allowed\"}") else when (val decision = authorization.authorize(bearer(exchange) ?: "", "read", Resource("protected", "ping"))) {
