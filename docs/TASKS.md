...
- [~] **UI / финальная Sentinel design system — Grok + GPT**, с 2026-08-23 07:56 CEST. Канонический PR: **#46** (`ui/sentinel-final-design-system-2026-08-23`). Дубли UI PR **#44** и **#45** закрыты без merge (порядок: один писатель / один PR). Initial compile-fix в `DesignTokens` устранил неверный resource reference; GPT дополнительно исправил provider-конфигурацию на стандартный Compose Google Fonts provider в commit `db3459c0723f6db437d8da79354305ae088a5506`, затем исправил scan-line: после 1000ms прохода линия оседает в постоянную 2dp левую акцентную границу в commit `866904ca2f5513b9f9d9de6883a635fc18404c4b`. Exact-head CI для `866904ca` запущен; задача остаётся `[~]` до завершения всех обязательных checks и acceptance.
...
- [~] UI/визуальный дизайн — **снято с «отложено»** по явному запросу пользователя 2026-08-23; исполнение в PR #46 (см. фокус выше). Не считать accepted без CI PASS.
