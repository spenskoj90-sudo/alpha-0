...
printf '%s' "$nonce" | python3 -c 'import sys,base64; sys.stdout.buffer.write(base64.urlsafe_b64decode(sys.stdin.read()+"=="))' > "$TMP/nonce.bin"
...
# Recovery/key rotation must invalidate the previous identity and require a fresh proof.
...
printf '%s' "$new_nonce" | python3 -c 'import sys,base64; sys.stdout.buffer.write(base64.urlsafe_b64decode(sys.stdin.read()+"=="))' > "$TMP/new-nonce.bin"
