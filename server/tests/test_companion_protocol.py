...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ugs_schema_version", "UGS_SCHEMA_MISMATCH"),
        ("adapter_contract_version", "ADAPTER_CONTRACT_MISMATCH"),
        ("core_protocol_version", "CORE_PROTOCOL_MISMATCH"),
        ("capability_profile", "CAPABILITY_PROFILE_MISMATCH"),
    ],
)
def test_handshake_mismatch_fails_closed(field: str, expected_reason: str) -> None:
    result = negotiate_handshake(
        handshake(**{field: "9.9"}),
        expected_ugs_schema="1.0",
        expected_adapter_contract="1.0",
