...
- [ ] **Block C — Companion/WoW/player experience/voice productization.** The launcher now has a real account/Companion increment: Core tokens remain in Electron main-process memory, a dedicated worker owns the Companion socket lifecycle, reconnect/backoff and heartbeats are bounded, browser-compatible socket auth avoids bearer-in-URL, paid access is checked before start and revalidated on live traffic, terminal entitlement revocation stops the worker, player-visible account/connection/degraded/kill-switch state exists, and launcher runtime tests run in Build & Test. Remaining Block C work is addon SavedVariables/local ingestion and persistence, conservative Core delivery of addon observations, complete player Overlay/voice runtime wiring, packaging/host acceptance, and exact WoW target evidence.
...
- [ ] **WoW-addon deterministic evidence.** Launcher runtime tests now exist in routine CI. Add repository-level addon ingestion/contract tests where technically possible; exact WoW 3.3.5a/private-server L3 behavior remains environment-unverified.
