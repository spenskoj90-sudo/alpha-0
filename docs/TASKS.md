...
- [ ] **Block C — Companion/WoW/player experience/voice productization.** The launcher has a real account/Companion runtime and passive WoW checkpoint increment: Core tokens remain in Electron main-process memory, a dedicated worker owns the Companion socket lifecycle, reconnect/backoff and heartbeats are bounded, browser-compatible socket auth avoids bearer-in-URL, paid access is checked before start and revalidated on live traffic, terminal entitlement revocation stops the worker, and player-visible account/connection/degraded/kill-switch state exists. Classic/Retail addons persist bounded `SentinelDB` checkpoints; launcher ingestion uses a strict non-evaluating Lua-subset parser, a disk-backed bounded FIFO and explicit `WOW_OBSERVATION_ACK`; Core normalizes accepted checkpoints through the conservative WoW adapter without granting `game:write`. SavedVariables are checkpoint evidence only, not realtime addon IPC. Remaining Block C work is complete player Overlay/voice runtime wiring, signed/packaged host acceptance and exact WoW target evidence.
