...
class RecommendationDelivery:
    ...
    def deliver(self, context: Mapping[str, Any], *, provider_id: str | None = None) -> tuple[DeliveredRecommendation, ...]:
        result: AIProviderResult = self._engine.recommend(context, provider_id=provider_id)
        return (DeliveredRecommendation(kind=result.kind, text=result.text, confidence=result.confidence, provenance=result.provenance, provider_id=result.provider_id, model_id=result.model_id),)
