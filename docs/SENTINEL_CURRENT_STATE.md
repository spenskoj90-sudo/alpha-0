...
- Launcher session/process/reconnect security, passive SavedVariables parser/queue/protocol behavior, overlay presentation isolation and voice consent/capture/provider/result boundaries have deterministic routine CI coverage once their exact integrating PR passes. This remains repository evidence; exact WoW addon lifecycle/host behavior, physical microphone/acoustic behavior and real packaged-overlay/voice latency are environment-unverified until exercised on the target runtime.
- End-to-end correlation propagation, deployed metrics/tracing composition and broader benchmark/failure-injection measurements remain separate implementation/evidence concerns when repository inspection does not prove them.

## 6. Internal architecture work remaining

The first six implementation passes plus Blocks A-C produced substantial internal foundations and player-runtime composition. The largest remaining internal target is now **Block D observability/performance/resilience productization** rather than missing voice wiring.

Largest remaining internal targets include:

1. Core-wide HTTP/request correlation and correlation continuity across Core ↔ Companion ↔ launcher operational telemetry where not already present.
2. Deployed/operator-facing metrics and tracing composition with bounded cardinality/privacy behavior rather than contract-only seams.
3. Broader benchmark and failure-injection evidence for transport/reconnect/queue/provider degradation, tied to exact SHAs and explicit budgets.
4. Remaining addon/launcher operational telemetry needed to diagnose real packaged-host behavior without exposing credentials or raw voice content.
...
Exact-environment evidence is deliberately separate from internal productization: exact WoW/private-server L3 behavior, selected production voice-provider behavior, physical microphone/driver/acoustic evidence, signed desktop packaging and real-host acceptance remain environment/Owner gates. Release signing/publication/deployment remain Owner-gated.
...
`UGS runtime validation → deterministic replay → conservative WoW adapter → Companion protocol/runtime → Policy Engine / Action Gateway → context/recommendation/confidence/provenance → Command Center/Overlay/Voice UX → observability/performance/resilience → real-device/integration acceptance`
