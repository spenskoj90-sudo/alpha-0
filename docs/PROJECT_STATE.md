...
GitHub Actions FTL authentication and Test Lab environment discovery are operational for project `sentinel-21ca9` using repository secrets `FIREBASE_SERVICE_ACCOUNT_JSON` and `FIREBASE_GCP_PROJECT_ID`. The Service Account `firebase-adminsdk-fbsvc@sentinel-21ca9.iam.gserviceaccount.com` has the `Firebase Test Lab Admin` role. In Build & Test run `32818319982`, Google authentication passed and `gcloud firebase test android models list` passed. The subsequent instrumentation execution reached the FTL API but was blocked because the Cloud Tool Results API was disabled in the project. `toolresults.googleapis.com` has now been enabled. The next action is a single targeted rerun of the existing FTL job; do not rerun the full CI unless required. Do not classify the earlier FTL failure as an APK/product test failure.

### FTL usage-control policy — RARE, MAXIMALLY INFORMATIVE RUNS

FTL is a cumulative validation resource and must not be triggered indiscriminately. Test Lab quotas are project-level and shared across instrumentation, Robo, Game Loop, and test matrices. Current documented no-cost limits are 15 test runs/day on Spark (10 virtual + 5 physical). Blaze begins with 60 minutes/day of virtual-device test time and 30 minutes/day of physical-device test time; usage above those limits is billable. Test runs, shards, and deflake retries can consume quota/resources. The project policy is therefore: prefer **rare, maximally informative runs** rather than frequent redundant runs; use a single targeted device/matrix for routine regression validation; batch broader device/API coverage into deliberate acceptance passes before important merges/releases; never rerun a valid test merely for repetition; perform cheap local/CI/unit/instrumentation/Core/PostgreSQL checks first and reserve FTL for real-device validation; before every FTL run verify that the APK and androidTest APK correspond to the intended commit SHA; preserve successful FTL results as evidence and reuse them while the tested APK remains unchanged. Source: Firebase Test Lab usage/quotas documentation, last updated 2026-08-19.
