...
- [x] Device Details revoke/rotate — предыдущая запись PR #38 о наличии backend endpoints была ошибочной; PR #40 реализовал реальные `/v1/devices/{device_id}/rotate` и `/v1/devices/{device_id}/revoke`, session-device linkage при `/v1/devices/bind`, ownership checks, fingerprint validation, session invalidation/replacement и Android persistence нового device/session. Security regression находится в `server/tests/test_device_session_lifecycle.py`. Дублирующие routes в `wow_api.py` удалены; канонические handlers остаются в `main.py`. Evidence: текущий `main` merge SHA `ebd344f5f42adab3f4b0dea7ee26f3af90b81c79`; Build & Test run `32626297494` — Core, PostgreSQL, Android build/tests и остальные code/build jobs PASS; FTL — известное GCP исключение.
