...
- [ ] **Block D — observability/performance/resilience productization.** Privacy scrubbing, budget contracts and deterministic recovery matrices exist. The authenticated loopback Companion runtime now converts launcher HEARTBEAT traffic into live Core heartbeat state plus a bounded correlated `HEALTH` response; launcher-side matching provides bounded local RTT count/min/max/average/p95 evidence and the read-only overlay can display p95 without exposing credentials/peer identity. Core-wide HTTP correlation, deployed metrics/tracing composition, broader benchmark/failure-injection measurements and remaining addon/launcher operational telemetry still require implementation/evidence where absent. Local RTT and deterministic tests are not production-network performance claims.
...
`Verified foundation → Block B productization → Companion/launcher/WoW/player/voice runtime → observability/performance/resilience runtime → Owner/external release gates`
