...
- **IMPLEMENTED / MAIN-VERIFIED:** approved shield/S/signal artwork is repository-managed for Web/Companion/high-value brand surfaces; adaptive/monochrome/notification glyph paths preserve the reduced brand DNA. Final launcher optical acceptance remains physical evidence.
- **IMPLEMENTED / MAIN-VERIFIED:** Web, Companion and Overlay use v3 geometry/state grammar. Overlay remains presentation-only and displays one meaningful message at a time. Companion kill switch requires explicit confirmation; voice remains explicit-consent push-to-talk.
...
- **IMPLEMENTED / CORE STAGING-VERIFIED:** account-level TOTP MFA spans Core, Android, Web and Companion. Password and federated first factors are intercepted by a short-lived server-side challenge before session issuance; encrypted TOTP seeds, replay-protected counters, one-time hashed recovery codes and session revocation on MFA policy changes are implemented. On 2026-09-19 the connected Neon pre-release database was directly observed with migration `014_account_mfa` applied; its stored checksum `572183e9e60ded7c2847fd6b8ea614f1fd51dbcacec0d11cf7d0c00c47e3cf4b` matches the current `server/migrations/014_account_mfa.sql` bytes, and the three MFA tables had RLS + FORCE RLS with the expected service-role policies. On 2026-09-21 the Owner-managed staging MFA secret was configured and the protected-main deployment produced by PR #319 reached `live` as Render deploy `dep-daoidqks728c73bndecg`. External staging acceptance run `35598223771` attempt 3 completed `STAGING_MFA_ACCEPTANCE=PASS` across enrollment, TOTP confirmation, enable-time session revocation, password first-factor MFA challenge, TOTP completion, recovery-code completion, recovery replay rejection, recovery-code rotation, disable-time session revocation and password-only login after disable. The staging provider catalog reported no enabled federated providers, so a live federated-provider MFA login remains environment-unverified; physical Android/Companion acceptance remains tracked separately.
