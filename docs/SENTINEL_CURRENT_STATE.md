...
- **IMPLEMENTED / ENVIRONMENT-UNVERIFIED:** account-level TOTP MFA spans Core, Android, Web and Companion. Password and federated first factors are intercepted by a short-lived server-side challenge before session issuance; encrypted TOTP seeds, replay-protected counters, one-time hashed recovery codes and session revocation on MFA policy changes are implemented. Staging remains environment-unverified until `SENTINEL_ACCOUNT_MFA_KEY` is configured through the deployment secret store, migration `014_account_mfa.sql` is observed applied, and real end-to-end MFA is exercised.
