...
def test_diagnostic_snapshot_preserves_schema_wire_alias() -> None:
    ...
    assert "schema_name" not in schema_properties
...
def test_validation_emits_no_schema_shadow_warning() -> None:
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        DiagnosticSnapshot.model_validate(diagnostic_snapshot_payload())

    shadow_warnings = [item for item in caught if "schema" in str(item.message).lower() and "shadow" in str(item.message).lower()]
