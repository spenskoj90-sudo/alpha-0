...
On 2026-09-19 an isolated target project `sentinel-pre-release-pg18` was created in `aws-eu-central-1` on PostgreSQL 18.6. All 15 repository migrations through `014_account_mfa` were applied to its `sentinel` database with checksum parity to the PG17 source; 39 application tables have RLS and all 39 also have FORCE RLS. Critical application tables are still empty, so the target is **SCHEMA-READY / DATA-IMPORT-READY** rather than migrated. The PG17 source remains intact, staging still points at the existing source, and data import plus connection-string cutover remain environment operations.
