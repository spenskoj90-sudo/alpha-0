...
store = MemoryStore()
# No wildcard DENY rule is needed: the engine itself returns DENY when no
# matching allow policy exists. Explicit DENY rules can still be added and win.
...
def prove_device(device_id: str, payload: DeviceProofRequest):
    ...
    if not device or device["state"] != "ACTIVE": raise HTTPException(status_code=401, detail="DEVICE_NOT_ACTIVE")
    challenge = store.challenges.get(session_hash(payload.challenge))
    if not challenge or challenge["device_id"] != device_id or challenge["consumed"] or challenge["expires_at"] < time.time(): raise HTTPException(status_code=401, detail="INVALID_OR_REPLAYED_CHALLENGE")
    if not fresh_request_timestamp(payload.timestamp): raise HTTPException(status_code=401, detail="STALE_REQUEST")
    ...
    store.sessions[session_hash(raw)] = {"user_id": device["user_id"], "device_id": device_id, "scopes": ["character:read", "game:write", "audit:read"], "roles": ["user"], "expires_at": expires.timestamp()}
...
def ingest_events(payload: EventBatchRequest, authorization: str = Header(...)) -> dict:
    ...
    if decision is Decision.DENY: raise HTTPException(status_code=403, detail=reason)
    accepted = 0
    for event in payload.events:
        if event.device_id != principal.device_id: raise HTTPException(status_code=403, detail="DEVICE_SCOPE_MISMATCH")
        if event.event_id in store.events: continue
        device = store.devices.get(event.device_id)
        if not device or event.sequence <= device["last_sequence"]: raise HTTPException(status_code=409, detail="SEQUENCE_REPLAY")
