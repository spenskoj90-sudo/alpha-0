...
def provision():
    ...
    reg = client.post(
        ...
        json={"user_id": "u1", "platform": "android", "public_key_der_b64": pub64, "fingerprint_sha256": fingerprint},
    )
...
def test_registration_requires_user_bound_enrollment():
    ...
    response = client.post(
        "/v1/devices/register",
        headers={"X-Enrollment-Token": "u2:secret"},
        json={"user_id": "u1", "platform": "android", "public_key_der_b64": "bad", "fingerprint_sha256": "0" * 64},
    )
...
def test_authorization_default_deny():
    ...
    denied = client.post(
        "/v1/authorize",
        headers={"Authorization": "Bearer " + session["session_token"]},
        json={"action": "admin:delete", "resource": "admin:system"},
    )
    assert allowed.status_code == 200
    assert denied.status_code == 403


def test_event_batch_is_idempotent():
    _, device, session = provision()
