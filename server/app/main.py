...
def _session_principal(token: str) -> Principal:
    ...
    return Principal(
        ...
        frozenset(record.get("roles", [])),
        frozenset(record.get("scopes", [])),
    )
...
def _require_entitlement(user_id: str, game_id: str) -> None:
    ...
    entitlements = store.entitlements.get(user_id, [])
    if not any(item.game_id == game_id and item.is_active(now) for item in entitlements):
...
def check_game_access(game_id: str, authorization: str = Header(...)) -> dict[str, object]:
    ...
    try:
        ...
        _audit(principal.user_id, principal.device_id, "game:access", game_id, Decision.DENY, str(exc.detail))
        raise
...
def admin_games(_: None = Header(default=None)) -> dict:
    require_admin()
    return {"games": [game.__dict__ for game in DIABLO_CATALOG]}
