...
- request-correlation propagation for Core HTTP calls, including one stable `X-Request-ID` across a 401 → refresh → retry chain;
...
- deterministic Node tests executed by the repository `Build & Test` workflow, including static Classic/Retail addon, overlay isolation, correlation and voice permission/capture contract checks.
...
Core HTTP calls generated by `CoreSessionManager` include a UUID `X-Request-ID`. An authenticated request that receives 401 reuses the same logical correlation id for its one-time refresh and retry. Request IDs are operational correlation only: they do not carry authorization or idempotency authority and are not exposed as session credentials.
...
See `docs/COMPANION_VOICE_RUNTIME_V1.md` for the normative runtime contract and `docs/BLOCK_D_OBSERVABILITY_RESILIENCE_V1.md` for the operational correlation/evidence boundary.
