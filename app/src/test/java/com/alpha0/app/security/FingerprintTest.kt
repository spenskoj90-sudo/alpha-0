...
class FingerprintTest {
    ...
    @Test
    fun sha256FingerprintContainsOnlyLowercaseHexCharacters() {
        val result = fingerprint("ALPHA-0".toByteArray())
    ...
    @Test
    fun sha256FingerprintMatchesHexEncoder() {
        val digest = MessageDigest
            .getInstance("SHA-256")
            .digest("ALPHA-0".toByteArray())

        assertEquals(Hex.encode(digest), fingerprint("ALPHA-0".toByteArray()))
    }
