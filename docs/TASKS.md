...
- [~] **UI / финальная Sentinel design system — Grok**, с 2026-08-23 07:56 CEST. Канонический PR: **#46** (`ui/sentinel-final-design-system-2026-08-23`). Дубли UI PR **#44** и **#45** закрыты без merge (порядок: один писатель / один PR). Compile-fix: `DesignTokens` ссылался на `com.alpha0.app.R.array.com_google_android_gms_fonts_certs` без `font_certs.xml` → Android APK FAIL; исправлено commit `6ab21d2ee3d8933e87b66d1830c9b9f78340c9e6` (certs из `androidx.compose.ui.text.googlefonts.R`). **Не закрывать `[x]`** до exact-head Android build PASS на HEAD PR #46 + review Claude. Merge только после green CI и явного OK пользователя.
