...
- [ ] **Block C — Companion/WoW/player experience/voice productization.** The launcher has a real account/Companion runtime, passive WoW checkpoint vertical and internally wired player overlay: Core tokens remain in Electron main-process memory, a dedicated worker owns the Companion socket lifecycle, reconnect/backoff and heartbeats are bounded, browser-compatible socket auth avoids bearer-in-URL, paid access is checked before start and revalidated on live traffic, terminal entitlement revocation stops the worker, and player-visible account/connection/degraded/kill-switch state exists. Classic/Retail addons persist bounded `SentinelDB` checkpoints; launcher ingestion uses a strict non-evaluating Lua-subset parser, a disk-backed bounded FIFO and explicit `WOW_OBSERVATION_ACK`; Core normalizes accepted checkpoints without granting `game:write`, composes deterministic player recommendations and emits explicit non-actionable `PRESENTATION` envelopes. The launcher validates presentations at worker and main-process boundaries and displays them in a sandboxed, non-focusable, mouse-ignoring always-on-top Electron overlay with bounded count/TTL. SavedVariables are checkpoint evidence only, not realtime addon IPC. Remaining Block C work is voice productization plus signed/packaged host acceptance and exact WoW target evidence.
...
`Verified foundation → Block B productization → Companion/launcher/WoW runtime → player overlay → voice UX → observability/performance runtime → Android transport debt → Owner/external release gates`
